...
--
-- Also, the coordinates used assume that `z` is up and down, which is different from Minecraft. Oops.
...
function getBearings(opts)
    ...
    local metadata = {
        -- Contains { primaryReferencePoint = ... }
        -- These are relative to index 1, 1
        layers = {},
    ...
    for y, row in pairs(secondaryReferencePointsMap) do
        for x, info in pairs(row) do
            ...
            table.insert(secondaryReferencePointsMap, { x = x, y = y })
        end
    end
...
function normalizeMap(opts)
    ...
    for name, info in pairs(labeledPositions) do
        key[info.char] = {
            type = 'label',
            name = name,
            behavior = info.behavior, -- might be nil
    ...
    local normalizeCell = function(cell, coord)
        ...
        if key[cell].type == 'label' then
            util.assert(key[cell].behavior == 'buildStartPos', 'For now, a label must have a behavior set to "buildStartPos"')
            util.assert(buildStartPos == nil, 'Two buildStartPos labels were found.')
    ...
    local verifySecondaryReferencePoints = function(layer, z)
        for i, refPoint in ipairs(metadata.secondaryReferencePoints) do
            ...
            local relX = metadata.layers[z].primaryReferencePoint.x + refPoint.x
            local relY = metadata.layers[z].primaryReferencePoint.y + refPoint.y
            if relX >= 1 and relY >= 1 and relX <= #layer[1] and relY >= #layer then
                util.assert(layer[y][x] == '.', 'Expected layer '..z..' to have a reference point at ('..x..','..y..').')
            end
        end
    end
    ...
    util.assert(buildStartPos ~= nil, 'A buildStartPos coord must be placed somewhere in the blueprint.')
    return {
...
--     layer (unless the "." would fall outside of its bounds).
...
function module.create(opts)
    ...
    local layersMetadata = getBearings({ layers = layers })
    local normalizeMapResult = normalizeMap({
        key = key,
        labeledPositions = labeledPositions,
        layers = layers,
        metadata = layersMetadata,
    })
    local normalizedLayers = normalizeMapResult.normalizedLayers
    local requiredResources = normalizeMapResult.costs
    local buildStartRelPos = normalizeMapResult.buildStartPos
    ...
    return {
        ...
        createTaskState = function(buildStartCmps)
            local absOriginPos = buildStartCmps.compassAt({
                forward = buildStartRelPos.y,
                right = -buildStartRelPos.x,
                up = buildStartRelPos.z,
                face = buildStartRelPos.face,
            }).pos
